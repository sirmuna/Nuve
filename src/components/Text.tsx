...
type Weight = 'extralight' | 'light' | 'normal' | 'medium' | 'semibold' | 'bold';
type Color =
  | 'ink'
  | 'accent'
  | 'muted'
...
type Tracking = 'tight' | 'normal' | 'wide' | 'wider' | 'widest' | 'custom-0_20em' | 'custom-0_25em' | 'custom-0_3em' | 'custom-0_4em';
type Leading = 'none' | 'tight' | 'snug' | 'normal' | 'relaxed' | 'loose' | 'custom-1_2' | 'custom-1_8';

interface TextProps extends React.HTMLAttributes<HTMLElement> {
  as?: 'p' | 'span' | 'div' | 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'em' | 'strong';
...
const Text: React.FC<TextProps> = ({
  ...
  if (size) {
    const sizeMap: Record<Size, string> = {
      ...
      'custom-0_45rem': 'text-[0.45rem]', 'custom-0_65rem': 'text-[0.65rem]', 'custom-0_7rem': 'text-[0.7rem]',
    };
  ...
  if (color) {
    const colorMap: Record<Color, string> = {
      'ink': 'text-ink', 'accent': 'text-accent', 'muted': 'text-muted', 'cream': 'text-cream',
      'accent-dark': 'text-accent-dark', 'cream-25': 'text-cream/25', 'cream-35': 'text-cream/35',
      'cream-40': 'text-cream/40', 'cream-50': 'text-cream/50', 'cream-85': 'text-cream/85',
      'accent-80': 'text-accent/80',
    };
  ...
  if (tracking) {
    const trackingMap: Record<Tracking, string> = {
      'tight': 'tracking-tight', 'normal': 'tracking-normal', 'wide': 'tracking-wide', 'wider': 'tracking-wider', 'widest': 'tracking-widest',
      'custom-0_20em': 'tracking-[0.20em]', 'custom-0_25em': 'tracking-[0.25em]', 'custom-0_3em': 'tracking-[0.3em]', 'custom-0_4em': 'tracking-[0.4em]',
    };
  ...
  if (leading) {
    const leadingMap: Record<Leading, string> = {
      ...
      'custom-1_2': 'leading-[1.2]', 'custom-1_8': 'leading-[1.8]',
    };
